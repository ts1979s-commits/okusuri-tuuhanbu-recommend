...
class FAISSRAGSystem:
    ...
    def __init__(self):
        ...
        # 方法1: プロキシ無効化での初期化（最優先）
        try:
            logger.info("試行 1/4: プロキシ無効化でのOpenAIクライアント作成...")
            
            # httpxクライアントを明示的にプロキシなしで作成
            import httpx
            custom_client = httpx.Client(proxies={}, verify=True)
            ...
            logger.info("プロキシなしクライアント作成完了、接続テスト実行中...")
            ...
            if test_response and hasattr(test_response, 'data'):
                ...
                init_success = True
                logger.info("✅ OpenAIクライアント初期化成功（プロキシ無効化方式）")
            else:
                logger.warning("接続テストでモデル一覧の取得に失敗")
        except Exception as e0:
            logger.warning(f"❌ プロキシ無効化方式初期化失敗: {type(e0).__name__}: {str(e0)}")
        
        if not init_success:
            # 方法2: 基本的な初期化（従来）
            try:
                logger.info("試行 2/4: 基本的なOpenAIクライアント作成...")
